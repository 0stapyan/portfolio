...
public class DijkstraWithDiagonals
{
    public (List<Point>, int, float, List<Point>) FindFastestTimePathUsingDiagonals(string[,] map, Point start, Point destination)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (Point neighbour in neighbours)
            {
                ...
                if (times[next.Item2] + GetTime(next.Item2, neighbour, map) < times[neighbour])
                {
                    ...
                    queue.Enqueue(times[neighbour], neighbour);
                }
            }
        }
